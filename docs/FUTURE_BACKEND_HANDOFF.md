...
| `InventoryItem` | `inventory_items` | `stockPackages`→`quantity_available` (>= 0 CHECK), `location`→`warehouse_location`, `nearestExpiry`→`expiry_date`, per-row `low_stock_threshold` (mock global const = 10). **M7H: `update_order_status` deducts `order_items.quantity` on transition to `delivered`, once (guarded by the `order_inventory_movements` ledger); insufficient stock blocks delivery; no auto-restore on un-deliver** |
...
| `ShowcaseLink` | `catalog_showcase_links` | M7H view-only "browse products" link: owner/admin issue a `token_hash`-only link (`insert/revoke_catalog_showcase_link`); anon reads the tenant catalog (NO customer, NO ordering) via `get_showcase_catalog` (token + rate limiter). RLS owner/admin read; RPC-only writes; no anon table access |
| — | `order_inventory_movements` | M7H append-only stock ledger written only by `update_order_status` on delivery; unique `(tenant_id, order_id, product_id, reason)` = idempotency guard. RLS owner/admin read; RPC-write only |
