...
| ✅ Logo upload error handling (M8E.1) | shared `src/lib/image-upload.ts` (client pre-check) + `try/catch/finally` in all upload handlers + `next.config` `serverActions.bodySizeLimit: 6mb` (default 1MB was < the 2/5MB image caps) + distinct `invalid` reason | done — no hang on reject; existing image kept on failure; no migration |
| ✅ Logo visibility (M8E.1) | tenant logo in admin shell (`admin/layout` → `getSupplier().logoUrl`), doc preview, shop/showcase headers (server-only `signTenantBrandingLogo`, own-tenant `<tenant>/branding/…` only); manufacturer logo in cards/chips | done — signed on read, fail-closed to name/initial; PDF logo embedding deferred |
