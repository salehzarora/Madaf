...
| — | `order_inventory_movements` | Append-only stock ledger written only by `update_order_status` / `update_order_items`. **M7I reasons: `order_reserved` (−, on confirm/preparing), `order_reservation_released` (+, on cancel), `order_edit_adjustment` (±, on edit). Partial-unique indexes guard reserve-once / release-once; net reserved = `-sum(quantity_delta) where reason in ('order_reserved','order_edit_adjustment')`.** RLS owner/admin read; RPC-write only |
