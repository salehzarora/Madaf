...
test("guard: the client drives the SESSION REDUCER — no ad-hoc anchor state", () => {
  ...
  assert.match(
    ...
    "atomic reset, in the handler — and the REDUCER allocates the generation",
  );
  assert.match(
    src,
    /timeZone: result\.resolvedTimeZone,/,
    "the SERVER's resolved zone binds the session — with NO fallback to the page prop",
  );
  assert.doesNotMatch(src, /resolvedTimeZone \?\?/, "no page-prop fallback survives");
