  ...
  return (
    <div className="mx-auto flex w-full max-w-2xl flex-col items-center px-4 py-14 text-center sm:px-6">
      ...
      <Card className="mt-6 w-full max-w-sm p-5">
        ...
        <p className="mt-1 text-2xl font-bold tracking-wide text-brand-700" dir="ltr">
          {orderNumber}
        </p>
      </Card>
      ...
      <div className="mt-10 flex flex-col gap-3 sm:flex-row">
        <Link
          href={`/${locale}/catalog`}
          className="inline-flex h-12 items-center justify-center rounded-field bg-brand-600 px-6 text-sm font-semibold text-white transition-colors hover:bg-brand-700"
        >
          {dict.orderSuccess.backToCatalog}
        </Link>
        <Link
          href={`/${locale}/admin/orders`}
          className="inline-flex h-12 items-center justify-center gap-2 rounded-field border border-line-strong bg-surface px-6 text-sm font-medium text-ink transition-colors hover:bg-surface-sunken"
        >
