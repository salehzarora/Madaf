...
| `migrations/20260712100000_tenant_tax_settings.sql` | M6B: `tenant_tax_settings` (INERT tax config) + owner/admin `get_tenant_tax_settings` / `upsert_tenant_tax_settings` RPCs. Deny-by-default RLS (owner/admin read; sales_rep/anon/non-member none); RPC-only writes; no secrets stored; `legal_invoicing_ready` does NOT enable issuing |
| `migrations/20260712110000_inert_legal_invoicing_schema.sql` | M6B: **INERT** future-legal schema (`legal_documents`/`legal_document_items`/`legal_invoice_sequences`/`legal_document_events`/`tax_authority_requests`/`tax_authority_responses`/`archival_records`/`signing_records`) + `legal_document_type`/`legal_document_status` enums + an issued-immutability guard trigger. RLS enabled + grants locked; **NO issuing/numbering RPC, no provider call, no route, no bucket** — unreachable by any app path. Not a legal tax invoice; see `docs/LEGAL_INVOICING_ARCHITECTURE.md` |
| `migrations/20260713100000_legal_numbering_skeleton.sql` | M6C: **DISABLED-by-default** legal numbering skeleton — `legal_numbering_settings` (single-row DB kill switch, default `false`, service-role only) + `draw_legal_document_number()` (owner/admin, `authorize_tenant`, atomic row-locked draw from `legal_invoice_sequences`, fail-closed behind the kill switch). Draws an INTERNAL non-legal preview (`DRAFT-LEGAL-YYYY-######`); issues NOTHING (no invoice, no legal_number on `legal_documents`, no allocation number, no provider call, no PDF). No UI/route calls it |
| `migrations/20260713110000_harden_legal_numbering_validation.sql` | M6C.1: recreate `draw_legal_document_number` with input validation — `p_year` defaults to current UTC year, must be `2000..2100` (`MDF61`); non-null `p_legal_entity_id` rejected (`MDF62`, no `legal_entities` table yet) so no arbitrary UUID enters `legal_invoice_sequences`. Order role→entity→year→kill switch→draw; invalid/disabled calls raise before the increment (no sequence row). Still DISABLED by default; issues nothing. **Rollback/gap:** committed draws atomic + not reused; a rolled-back txn rolls back its increment (reuse across rolled-back attempts possible — fine for the internal preview skeleton; real issuance defines the gap policy later) |
| `migrations/20260714100000_sandbox_legal_orchestration.sql` | M6E: SANDBOX-ONLY legal orchestration. Adds `sandbox`/`legal_effective`/`non_legal_notice`/`provider_mode` markers to `legal_documents` (+ `sandbox` to `tax_authority_requests`, `sandbox`/`legal_effective` to `tax_authority_responses`) with **HARD CHECK `legal_effective = false`** + sandbox-only `provider_mode`; and `sandbox_issue_legal_document()` (owner/admin, fail-closed behind the M6C DB kill switch + `provider_mode=sandbox` only) which writes ONLY a `draft_internal` sandbox row (`legal_number`/`allocation_number` stay NULL) + a redacted request/response log pair. Issues NOTHING real; a legally-effective row is impossible |
| `migrations/20260714110000_harden_sandbox_orchestration_rpc.sql` | M6E.1: harden `sandbox_issue_legal_document` (it is EXECUTE-granted to authenticated). DROPs the old JSON-accepting overload and recreates a 5-arg RPC that enforces every write gate in SQL: tenant tax readiness (`legal_invoicing_ready=true` → `MDF73`), the M6C numbering draw INSIDE the RPC (kill switch off → fail; duplicate idempotency fails **before** draw → no increment, `MDF71`), sandbox-only mode (`MDF72`), and persists NO caller JSON (SQL-generated minimal sandbox payloads; idempotency key hashed). Still sandbox-only / non-legal |
| `migrations/20260714120000_sandbox_archival_signing.sql` | M6F: SANDBOX write-once archival + signing (non-legal). Adds `sandbox`/`legal_effective`/`non_legal_notice`/`provider_mode`/`content_sha256`/`idempotency_key` markers to `archival_records`/`signing_records` with HARD CHECK `legal_effective=false`, sandbox-only `provider_mode`, sandbox-placeholder signature/algorithm/cert, `unique (tenant_id, legal_document_id)` (write-once) + an immutability trigger. And `sandbox_archive_and_sign_legal_document()` (owner/admin, fail-closed DB kill switch, validates the target is an M6E sandbox non-legal doc `MDF75`, no caller JSON — canonical payload + SHA-256 in SQL, idempotency hashed, write-once `MDF74`). Not a real archive/signature; issues nothing |
