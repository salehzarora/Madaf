...
- **sales_rep scoping (M4D):** `can_access_customer(tenant, customer)`
  gives owner/admin every customer in the tenant and a sales_rep only their
  assigned ones. It backs the `customers` SELECT policy (a rep sees only
  assigned customers) and `create_order_request` (a rep may order only for
  an assigned customer — no fall-back to all). Assignments are managed by
  owner/admin via `assign_customer_to_rep` / `unassign_customer_from_rep`.
